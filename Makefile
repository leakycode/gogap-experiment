...
$(APP): \
	...
	$(APPDIR)/Contents/Resources/Base.lproj/MainWindow.nib \
	...
	$(APPDIR)/Contents/Resources/public

$(APPDIR)/Contents/MacOS/$(APP): $(GOSRC) src/
	-mkdir -p "`dirname \"$@\"`"
	cd src/ && CC=$(CC) go build -ldflags="-linkmode=external" -o "$(CURDIR)/$@"

$(APPDIR)/Contents/Resources/public: public/
	-cp -r $< "$@"

$(APPDIR)/Contents/Resources/Base.lproj/MainWindow.nib: res/MainWindow.xib
	-mkdir -p "`dirname \"$@\"`"
